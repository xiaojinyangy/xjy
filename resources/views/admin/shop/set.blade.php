...
@section('body')
    ...
    <form class="layui-form layui-form-pane" action="">
        <div class="layui-form-item" style="display: inline-block">
            ...
            <div class="layui-input-block">
                <select name="mouth_id" lay-filter="mouth" id="mouth">
                    <option value="">全部</option>
                    @foreach($mouth_list  as $v)
                        <option value="{{$v['id']}}" {{$v['id'] == $result['mouth_id'] ? 'selected': ''}}>{{$v['mouth_name']}}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="layui-form-item">
            <div class="layui-form-item" pane="">
                ...
                <div class="layui-input-block">
                    <input type="radio" name="status" value="0" title="通过" {{$result['status'] == 1 ?  "checked" : ""}}>
                    <input type="radio" name="status" value="1" title="待审核" {{$result['status'] == 2 ?  "checked" : ""}} >
                    <input type="radio" name="status" value="2" title="通过" {{$result['status'] == 2 ?  "checked" : ""}} >
                </div>
            </div>
        </div>
